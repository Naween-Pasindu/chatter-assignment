...
public class ChatServer {
     ...
     * already in use.
     */
    private static Hashtable<String,PrintWriter> names = new Hashtable<String,PrintWriter>();
    ...
    private static class Handler implements Runnable {
        ...
        public void updateNameList() {
            ...
            for (Map.Entry<String,PrintWriter> entry : names.entrySet()) {
                String key = entry.getKey();
                PrintWriter writer = entry.getValue();
                storeName = "";
                for(String name:names.keySet()) {
                	if(!key.strip().equalsIgnoreCase(name)) {
                		storeName += ":"+name;
                	}
                }
                writer.println("NAME" + storeName);
            }
        }
        public void run() {
            ...
            try {
                ...
                while (true) {
                    ...
                    if(input.startsWith("DIRECT")) {
                        if(input.length()>6) {
                            ...
                            String[] messegeArray = temp.split("#");
                            temp = messegeArray[0];
                            String messege = messegeArray[1];
                            for(String nameTemp:temp.split(":")) {
                            	if(names.containsKey(nameTemp)) {
                            		PrintWriter writer = names.get(nameTemp);
                            		writer.println("MESSAGE " + name + ": " + messege);
                            	}
                            }
                        	if(names.containsKey(name)) {
                        		PrintWriter writer = names.get(name);
                        		writer.println("MESSAGE " + name + ": " + messege);
                        	}
                        }
                    }else {
                ...
                for (PrintWriter writer : writers) {
                    writer.println("MESSAGE " + name + " left the conversation.");
                }
